...
const EventDetails = () => {
  ...
  const { data: province } = useLocation("province", eventFetched && event.province);
  const { data: regency } = useLocation("regency", eventFetched && event.regency);
  const { data: district } = useLocation("district", eventFetched && event.district);
  ...
  const isAlreadyAttend = eventFetched && event.attendees.filter((attendee) => attendee.userId === userId).length > 0;
