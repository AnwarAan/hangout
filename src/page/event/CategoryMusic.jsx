...
const CategoryPage = () => {
    ...
    const { data, isLoading } = useQuery([`/category/${categoryMusic}`], async () => {
        try {
          const res = await getAPI(`event?category=music`)
            ...
            <div className="w-full bg-secondary mt-4 h-[250px] rounded-md">
                <div className="w-full mx-auto flex items-center h-full">
                    <div className=" flex flex-col justify-center px-8 gap-2">
                        <h1 className="text-6xl font-extrabold text-primary max-w-[800px]">{`Music Events`}</h1>
